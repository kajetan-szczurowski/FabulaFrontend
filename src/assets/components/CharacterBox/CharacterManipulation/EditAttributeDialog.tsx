    ...
    return(
        <dialog ref = {dialogRef} className = 'character-box-button'>
            ...
            <div className = 'spacer' ref = {secondaryDivRef} style = {{display: 'none'}}>
                <span className='are-you-sure-question'>{translate('Are you sure? This action is irreversible.')}</span>
                
                <div className = 'spacer'>
                    <button className='character-box-button main-text' onClick = {hideDeleteQuestion}>{translate('No')}</button>
                    <button className='delete-button' style = {DELETE_MARGIN_LEFT_STYLE} onClick = {handleDelete}>{translate('Yes, delete')} <span>{toDeleteLabel}</span>.</button>
                </div>
            </div>
            </div>
        </dialog>
